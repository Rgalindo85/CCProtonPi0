...
bool CCProtonPi0::createTrackedParticles(Minerva::PhysicsEvent *event) const
{
  ...
  for (unsigned int p = 0; p < prongs.size(); p++){
    ...
    // Do NOT use prongs which have ODMatch
    if (prongs[p]->OdMatch()){
...
bool CCProtonPi0::getPionProngs(Minerva::PhysicsEvent *event) const
{
  debug() << " Entering to getPionProngs " << endmsg;
	...
	for (ProngVect::iterator itHadron = Pion_hadron_prongs.begin(); itHadron != Pion_hadron_prongs.end(); ++itHadron){
	  ...
	  if (!hadronProng->filtertaglist()->filterTagExists("PrimaryHadron") ){
	    debug() << "Bad Prong, it is not a PrimaryHadron " << endmsg;
	    continue;
	  }
...
void CCProtonPi0::GetBlobDirection(Minerva::PhysicsEvent *event, Minerva::IDBlob *blob) const
{
  ...
  double centroid_Y = (-1.7321/3)*(pos_centroid[1] - pos_centroid[2]);
...
bool CCProtonPi0::PreFilterPi0(Minerva::PhysicsEvent *event, Minerva::GenMinInteraction* truthEvent ) const
{
    ...
    const double max_Evis_Other = 50000; // I'm not putting a max value cut,  I just put test for a high value
    ...
    for ( SmartRefVector<Minerva::IDCluster>::iterator iter_cluster = unusedClusters.begin(); iter_cluster != unusedClusters.end(); ++iter_cluster ){
    ...
    for (   SmartRefVector<Minerva::IDCluster>::iterator iter_cluster = nearVertexClusters.begin(); iter_cluster != nearVertexClusters.end(); ++iter_cluster ){
