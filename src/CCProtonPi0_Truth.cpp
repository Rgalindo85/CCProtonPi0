...
//              FS Particles: At least 1 Pi0, 0, 1 and more the 1 charged pions (Proton and Neutrons are OK)
...
bool CCProtonPi0::tagSignal(Minerva::GenMinInteraction* truthEvent) const
{
    ...
    //                 FS Particles: muon, pi0, X (and Mesons)
    //--------------------------------------------------------------------------
    bool isCCNeutrino = isSignal_CCNeutrino(truthEvent);
    bool isFSGood = (N_pi0 > 0);
    bool Signal_0 = (isFSGood && N_chargedPion == 0);
    bool Signal_1 = (isFSGood && N_chargedPion == 1);
    bool Signal_2 = (isFSGood && N_chargedPion > 1);
    ...
    bool isSignal_0 = isSignal && Signal_0;
    bool isSignal_1 = isSignal && Signal_1;
    bool isSignal_2 = isSignal && Signal_2;

    debug() << "isSignal = " << isSignal << endmsg;
    if (isSignal){
      debug() << "isSignal_0 = " << isSignal_0 << endmsg;
      debug() << "isSignal_1 = " << isSignal_1 << endmsg;
      debug() << "isSignal_2 = " << isSignal_2 << endmsg;
    }

    truthEvent->filtertaglist()->setOrAddFilterTag( "isSignal",   isSignal);
    truthEvent->filtertaglist()->setOrAddFilterTag( "isSignal_0", isSignal_0);
    truthEvent->filtertaglist()->setOrAddFilterTag( "isSignal_1", isSignal_1);
    truthEvent->filtertaglist()->setOrAddFilterTag( "isSignal_2", isSignal_2);
