    ...
    MinervaAnalysisTool( type, name, parent ) 
{
    ...
    declareProperty("KeepAfter_VertexCuts", m_keepAfter_vertex_cuts = true);
    declareProperty("KeepAfter_MuonCuts", m_keepAfter_muon_cuts = true);
    declareProperty("KeepAfter_MichelCuts", m_keepAfter_michel_cuts = true);
    declareProperty("KeepAfter_ProtonCuts", m_keepAfter_proton_cuts = true);
    declareProperty("KeepAfter_PionCuts", m_keepAfter_pion_cuts = true);        // I have to change this in the opts file
    declareProperty("KeepAfter_Pi0Cuts", m_keepAfter_pi0_cuts = true);
...
StatusCode CCProtonPi0::initialize()
{
    ...
    //declareIntEventBranch("has_EndPoint_Michel",            -1);
    //declareIntEventBranch("hasSecondaryMichel",             -1);
    //declareIntEventBranch("has_EndPoint_Michel_MultiTrack", -1);
    //declareIntEventBranch("has_EndPion_Vtx",                -1);
...
StatusCode CCProtonPi0::reconstructEvent( Minerva::PhysicsEvent *event, Minerva::GenMinInteraction* truthEvent ) const
{
    ...
    //I'm checking the existence of Michel Electrons, not applying a cut
    ...
    //==========================================================================
    // Charged Pion Reconstrucction
    //==========================================================================
    debug() << "START: Pion Reconstruction" << endmsg;
    ...
    if (nPrimaryProngs > 1){
    	...
    	if ( !foundPion){
            debug() << "Didn't find any contained in the tracker bit-positive prong with a pion particle! " << endmsg;
            event->setIntData("Cut_Pion_None", 1);
            // I'm not skipping the event, even w/o a pion is my signal.
    	}
    }

    //Set Pion Kinematics
    if ( m_PionParticles.size() > 0){
        bool pionFilled = setPionData(event);
        if ( !pionFilled ){
            debug() << "Pion Momentum is NaN, rejecting event!" <<endmsg;
            event->setIntData("Cut_Pion_Bad", 1);
        }
    }else{
        debug()<<"No Pion Particle, Setting -9.9 to m_pion_4P"<<endmsg;
        m_pion_4P.SetPxPyPzE(-9.9,-9.9,-9.9,-9.9);
    }
        
    debug() <<"Found "<<m_PionParticles.size()<<" Good Pion Candidates!"<<endmsg;
    event->setIntData("nPionCandidates", (int)m_PionParticles.size());
    
    //--------------------------------------------------------------------------
    // Debugging: Check values
    debug()<<"m_pion_4P = ( "<<m_pion_4P.px()<<", "<<m_pion_4P.py()<<", "<<m_pion_4P.pz()<<", "<<m_pion_4P.E()<<" )"<<endmsg;
    //--------------------------------------------------------------------------
    debug() << "FINISH: Pion Reconstruction" << endmsg;
...
bool CCProtonPi0::createTrackedParticles(Minerva::PhysicsEvent *event) const
{
  ...
  for (unsigned int p = 0; p < prongs.size(); p++){
    ...
    if (prongs[p]->filtertaglist()->filterTagExists(tag) ){
      if (prongs[0]->filtertaglist()->checkFilterTag(tag, pass)){
...
bool CCProtonPi0::createTrackedParticles(Minerva::PhysicsEvent *event) const
{
  ...
  // Create a container of prongs to be hadron candidates, skipping the muon prong
  ProngVect hadron_prongs;
  ProngVect primaryProngs = event->primaryProngs();
  double hadron_visible_energy = 0.0;
  bool makeParticles = false;

  // loop over primary prongs and select the hadron prongs candidates
  for (ProngVect::iterator itProngs = primaryProngs.begin(); itProngs != primaryProngs.end(); ++itProngs ){
	...
	(*itProngs)->setIntData("hasEndPointMichel", 1);
      ...
      debug() << "The prong has " << (*itProngs)->minervaTracks().size() << " Tracks" << endmsg; 
      /*
      if ( (*itProngs)->minervaTracks().size > 1 ){
	
	debug() << "Loop over the tracks in the prong" << endmsg;
	for (SmartRefVector<Minerva::Track>::iterator itTracks = (*itProngs)->minervaTracks().begin(); itTracks() != (*itProngs)->minervaTracks().end()-1; ++itTracks){
	  ...
	  if ( foundMichel){
	    (*itProngs)->setIntData("hasSecondaryMichel", 1);
	  }
...
/*
  ...
  event->setIntData("N_Primary_Hadrons", hadron_prongs.size());
  event->setDoubleData("hadron_visible_energy", hadron_visible_energy);
 
  debug() << "Exiting from Create Tracked Particles: " << "makeParticles is " << makeParticles << " ... " << endmsg;
...
bool CCProtonPi0::getPionProngs(Minerva::PhysicsEvent *event) const
{
  debug() << " Entering to getPionProngs " << endmsg;
	...
	bool isPionExist = false;
	//std::vector<int> hadron_trackNodes;

	for (ProngVect::iterator itProng = prongs.begin(); itProng != prongs.end(); ++itProng){
	  // Skip Muon Prong
	  if( (*itProng)->filtertaglist()->filterTagExists("PrimaryMuon") ){
	    debug() <<"From getPionProngs ===> Muon Prong, skipping this prong! "<< endmsg;
	    continue;
	  }
	...
	debug() << "The event has " << Pion_hadron_prongs.size() << " hadron prongs " << endmsg;

	for (ProngVect::iterator itHadron = Pion_hadron_prongs.begin(); itHadron != Pion_hadron_prongs.end(); ++itHadron){
	  SmartRef<Minerva::Prong> hadronProng   = *itHadron;
	  SmartRef<Minerva::Particle> pionPart   = (Minerva::Particle*)NULL;
	  SmartRef<Minerva::Particle> protonPart = (Minerva::Particle*)NULL;

	  if (hadronProng->hasIntData("hasEndPointMichel") ){
	    debug() << "From getPionProngs ===> there is a prong with a Michel endPoint, pi+ candidate!!" << endmsg;
	    // fill some info about the michels
	  }
...
//===============================================================================================================
// Get the best particle in the Prong
//===============================================================================================================
void CCProtonPi0::getBestParticle(SmartRef<Minerva::Prong> prong, SmartRef<Minerva::Particle>& particle, Minerva::Particle::ID partType) const
{
	debug()<<"Entering  to getBestParticle"<<endmsg;

	//bool hasBestParticle = false;
	...
	debug()<<"Exiting to getBestParticle"<<endmsg;
	
	//return hasBestParticle;
}

/*bool CCProtonPi0::getBestParticle(Minerva::ProngVect& prong, SmartRef<Particle>& particle, Minerva::Particle::ID partType) const
{
	debug() << "Trying to find a pion " << endmsg;
	SmartRefVector<Minerva::Particle> partHypVec = prong->particles();
	SmartRefVector<Minerva::Particle>::iterator itPart;

	// get prong's best dEdX particle hypotheses;
	double largest_score = -1.0;

	bool pass_score_cut = false;
...
//==============================================================================
void CCProtonPi0::correctProtonProngEnergy(  SmartRef<Minerva::Prong>& protonProng, double& p_calCorrection, double& p_visEnergyCorrection ) const
{
    //-- initialize
    double calEnergy = 0.0;
    double visEnergy = 0.0;
...
bool CCProtonPi0::ConeBlobs(Minerva::PhysicsEvent *event, Minerva::GenMinInteraction *truthEvent ) const
{
    ...
    if ( !isAngleScan) {
	    ...
	    //----------------------------------------------------------------------
	    // Try to recover showers with different methods
	    // 1) Bassically this is going to repeat the thecnique for 3 blobs
	    //  Let's see what happends.....
	    //----------------------------------------------------------------------
	    if (m_TrytoRecover_4Shower && foundBlobs.size() == 4){
	        bool areAll4ShowersGood = Save_4ShowerInfo(foundBlobs,event);
	        if (areAll4ShowersGood && truthEvent && m_study_shower_recovery){
	            Save_4ShowerTruthMatch(foundBlobs, truthEvent);
	        }

	        debug()<<"Trying to Recover 4 Shower Events"<<endmsg;
	        if (!areAll4ShowersGood && m_recoverShower_Direction){
	            debug()<<"Not All of the 4 Showers direction is Good"<<endmsg;
	            debug()<<"Trying to recover 2 GOOD showers"<<endmsg;
	            isBlobsRecovered = RecoverShowers_Direction(foundBlobs, event);
	            isBlobsRecovered_Direction = isBlobsRecovered;
	        }
...
bool CCProtonPi0::PreFilterPi0(Minerva::PhysicsEvent *event, Minerva::GenMinInteraction* truthEvent ) const
{
    ...
    const double max_Evis_Other = 10000; // energy bigger than 2500 MeV must be ignored - DIS Events
...
void CCProtonPi0::DiscardFarTracks(Minerva::PhysicsEvent *event) const
{
    ...
    for (SmartRefVector<Minerva::Track>::iterator t = tracks.begin(); t != tracks.end(); ++t) {
        ...
        if (distance < 50) ++trackMultiplicity;
        if (distance > 50) farTracks.push_back(*t);
    }
...
void CCProtonPi0::Save_4ShowerTruthMatch( std::vector<Minerva::IDBlob*> &foundBlobs, Minerva::GenMinInteraction* truthEvent) const
{
  ...
  shower4Info.ParseTruth(shower4Clusters, fTrajectoryMap);
...
bool CCProtonPi0::isShowerGood(Minerva::IDBlob* shower, Minerva::PhysicsEvent* event) const
{
    ...
    if (goodPosition || goodDirection) return true;
    else return false;
    //return isShowerGood;
}
